<template>
  <main class="lg:h-screen bg-center bg-cover text-red-100 lg:flex bg-[#392121]" :style="{backgroundImage}">
    ...
    <section
      ...
      <div class="">
        <h1 class="text-2xl md:text-4xl lg:text-6xl text-center lg:text-left m-5">
          Recent Work
        </h1>
        <div class="projects flex flex-wrap">
          <router-link :to="{name: 'project', params:{projectId: project.id}}" v-for="project in projects"
            ...
            <div class="absolute inset-x-0 bottom-0 bg-black/60 visible group-hover:visible md:invisible p-2">{{
                project.title
              }}
            </div>
          </router-link>
